...
const InternalExamEvaluation = () => {
  ...
  const handleEvaluation = async (index) => {
    ...
    try {
      let student = await axiosPrivate.get(
        "/academic/getStudentDetailInternal",
        {
          params: {
  ...
  const SubmitStudentMarks = async () => {
    if (validateMark(students)) {
      if (window.confirm("Are you sure you want to submit the final marks")) {
        console.log(students);
        try {
          let response = await axiosPrivate.post(
            "/academic/addInternalMarks",
            students
          );
  ...
  const handleEditEvaluation = async (index) => {
    try {
      ...
      if (student.data.length === 0) {
        alert("Assement is is not assessed");
      } else {
        console.log("Student");
        console.log(student);
        let stud = await student.data.map((val) => {
...
export default InternalExamEvaluation;
